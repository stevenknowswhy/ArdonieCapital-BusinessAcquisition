// Ardonie Capital - Main Footer Component
// Footer with role-based CMS section for Blog Editors/Contributors

window.ArdonieFooter = {
    template: `
    <!-- Footer -->
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div class="lg:col-span-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div id="footer-cms-section" class="hidden">
                            <h3 class="text-sm font-semibold mb-3 text-white">Content Management</h3>
                            <ul class="space-y-1.5">
                                <li><a href="/dashboard/content-management.html" class="text-slate-400 hover:text-white transition-colors text-sm">Blog Dashboard</a></li>
                                <li><a href="/dashboard/content-management.html#create" class="text-slate-400 hover:text-white transition-colors text-sm">Create New Post</a></li>
                                <li><a href="/dashboard/content-management.html#manage" class="text-slate-400 hover:text-white transition-colors text-sm">Manage Content</a></li>
                                <li><a href="/blog/content-guidelines.html" class="text-slate-400 hover:text-white transition-colors text-sm">Content Guidelines</a></li>
                            </ul>
                        </div>

                        <!-- Account & Support (Default - Hidden when CMS is shown) -->
                        <div id="footer-account-section">
    ...
    init: function() {
        ...
        this.render();
        this.updateCMSFooterVisibility();
        
        // Listen for authentication changes
        window.addEventListener('storage', (e) => {
            if (e.key === 'ardonie_user_session' || e.key === 'ardonie_auth_status') {
                this.updateCMSFooterVisibility();
            }
        });
    },
    ...
    // Update CMS footer section visibility
    updateCMSFooterVisibility: function() {
        console.log('🦶 Footer: Updating CMS section visibility...');
        ...
        const cmsSection = document.getElementById('footer-cms-section');
        const accountSection = document.getElementById('footer-account-section');
        
        if (isAuthenticated && user) {
            try {
                const userData = JSON.parse(user);
                
                // Check if user has blog editor/contributor roles (but NOT admin roles)
                ...
                console.log('🦶 Footer: CMS visibility check:', {
                ...
                if (shouldShowCMS) {
                    console.log('✅ Footer: Showing CMS section');
                    if (cmsSection) cmsSection.classList.remove('hidden');
                    if (accountSection) accountSection.classList.add('hidden');
                } else {
                    console.log('❌ Footer: Hiding CMS section');
                    if (cmsSection) cmsSection.classList.add('hidden');
                    if (accountSection) accountSection.classList.remove('hidden');
                }
            } catch (error) {
                console.error('❌ Footer: Error parsing user data:', error);
                if (cmsSection) cmsSection.classList.add('hidden');
                if (accountSection) accountSection.classList.remove('hidden');
            }
        } else {
            console.log('❌ Footer: Not authenticated, hiding CMS section');
            if (cmsSection) cmsSection.classList.add('hidden');
            if (accountSection) accountSection.classList.remove('hidden');
        }
    },
...
window.ArdonieFooter = ArdonieFooter;
