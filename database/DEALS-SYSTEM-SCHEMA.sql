...
-- Fix generate_deal_number function (drop dependent triggers first, then function, to avoid dependency errors)
-- CORRECTED: Drop both triggers that depend on the functions
DROP TRIGGER IF EXISTS trigger_generate_deal_number ON deals;
DROP TRIGGER IF EXISTS set_deal_number_trigger ON deals;
DROP FUNCTION IF EXISTS public.generate_deal_number();
...
-- Fix set_deal_number function (drop first to allow redefinition)
DROP FUNCTION IF EXISTS public.set_deal_number();
...
-- Create trigger for set_deal_number (recreate with the correct name)
-- CORRECTED: Use the actual trigger name from the error message
DROP TRIGGER IF EXISTS set_deal_number_trigger ON deals;
