-- INSERT REMAINING BLOG POSTS (POSTS 4-6) - FIXED VERSION
-- Fixed version with name-based category lookup instead of hardcoded UUIDs
-- Run this AFTER step3-insert-first-3-posts-FIXED.sql completes successfully
-- Project: pbydepsqcypwqbicnsco

-- ============================================================================
-- INSERT REMAINING 3 BLOG POSTS (WITH ROBUST VERIFICATION)
-- ============================================================================
...
BEGIN
    ...
    -- Get category IDs by name (robust approach instead of hardcoded UUIDs)
    SELECT id INTO financing_id 
    FROM blog_categories 
    WHERE name = 'Financing';
    
    SELECT id INTO success_stories_id 
    FROM blog_categories 
    WHERE name = 'Success Stories';
    
    SELECT id INTO legal_compliance_id 
    FROM blog_categories 
    WHERE name = 'Legal & Compliance';

    -- Verify we found all required categories
    IF financing_id IS NULL THEN
        RAISE EXCEPTION 'Financing category not found by name. Available categories: %', 
            (SELECT string_agg(name, ', ') FROM blog_categories);
    END IF;
    
    IF success_stories_id IS NULL THEN
        RAISE EXCEPTION 'Success Stories category not found by name. Available categories: %', 
            (SELECT string_agg(name, ', ') FROM blog_categories);
    END IF;
    
    IF legal_compliance_id IS NULL THEN
        RAISE EXCEPTION 'Legal & Compliance category not found by name. Available categories: %', 
            (SELECT string_agg(name, ', ') FROM blog_categories);
    END IF;
    ...
    -- Insert remaining blog posts (4-6) using dynamic category IDs
    ...
    (
        ...
        financing_id, -- Using dynamic category ID
        9,
    ...
    (
        ...
        success_stories_id, -- Using dynamic category ID
        7,
    ...
    (
        ...
        legal_compliance_id, -- Using dynamic category ID
        11,
    ...
    -- Success message
    RAISE NOTICE '✅ Remaining 3 blog posts inserted successfully with category IDs: %, %, %', 
        financing_id, success_stories_id, legal_compliance_id;
...
SELECT 
    '✅ ALL POSTS COMPLETE' as status,
    COUNT(*) as total_posts
FROM content_pages;

-- List all posts with categories
SELECT 
    'FINAL_POST_LIST' as result,
    cp.title,
...
-- Check posts per category
SELECT 
    'POSTS_PER_CATEGORY' as result,
    bc.name as category_name,
...
SELECT '🎉 ALL 6 BLOG POSTS CREATED SUCCESSFULLY!' as final_status;
SELECT 'CMS is now populated with complete blog content' as summary;
SELECT 'You can now run the CMS verification test' as next_step;
